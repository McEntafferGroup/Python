...
def make_times():
    global year_i, month_i, day_i, ndays_i, offset_i
    global times, window_x, window_y, ndays, offset
    global step
    ...
    day, 0, 0, 0), scale='utc')-offset*u.hour
...
def get_SMO_pos():
    global lat_i, long_i, RA_i, DEC_i, times, offset, tname_i, option, places
    ...
    coords_s = coordinates.get_sun (times).gcrs
    """Moon's RA and DEC at the location, throughout the given time"""
    coords_m = coordinates.get_moon(times+offset*u.hour, location=loc).gcrs
...
def get_angles():
    ...
    alt_s = (coords_s.transform_to(coordinates.AltAz(obstime=times+offset*u.hour, location=loc)).alt.degree<sun_alt)
    alt_m = (coords_m.transform_to(coordinates.AltAz(obstime=times+offset*u.hour, location=loc)).alt.degree<moon_alt)
    """object's altitude in degrees"""
    true_alt_o = (coords_o.transform_to(coordinates.AltAz(obstime=times+offset*u.hour, location=loc)).alt.degree)
...
def plot_it():
    ...
    plt.xlabel('Date at {}'.format(window_x[0].time()))
...
def reset_the_GUI():
    ...
    global offset, dist_moon, dist_sun
    
    """reset all buttons and variables in order so the GUI looks nice"""
    del offset, ndays, times, window_x, window_y
...
Label(master, bg='#FFE100', text='LOCAL DATE OF START TIME (YYYY:MM:DD):').grid(row=row_counter)
...
"""increase RC after every row"""
row_counter+=1
"""make label and entry for offset from GMT"""
Label(master, bg='#FFE100', text='Hour offset from GMT:').grid(row=row_counter)
offset_i = Entry(master, bg='black', fg='#FFE100')
offset_i.grid(row=row_counter, column=1)
"""increase RC after every row"""
row_counter+=1
...
warning_label = Label(master, text=' CLEAR DATA AFTER EACH RUN OF GUI TO NOT CONTAMINATE SUBSEQUENT DATA SETS \n\nANGLES MUST BE INPUT INTO EVERY FIELD EVEN IF UNCHECKED TO AVOID ERRORS \n\n CLOSE ALL PREVIOUS PLOTS BEFORE CREATING NEW ONES SO TIME AXES ARE LEGIBLE', bg='red', relief='ridge', bd=5)
