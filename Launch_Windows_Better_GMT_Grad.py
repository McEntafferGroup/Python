...
def get_angles():
    ...
    global times, PI, loc
    global dist_sun, dist_moon, alt_s, alt_m, alt_o, obj_alt, sun_angle, moon_angle, sun_alt, moon_alt, dist_s, dist_m, coords_s, coords_m
    """read in from entry boxes"""
    ...
    alt_m = (moon_alt-coords_m.alt.degree)
    """whether or not the altitude is high enough"""
    #alt_o = loc.target_is_up(times,coords_o, horizon=obj_alt*u.deg)
    alt_o = (coords_o.transform_to(coordinates.AltAz(obstime=times, location=coordinates.EarthLocation(lat=lat*u.deg, lon=long*u.deg))).alt.degree)-obj_alt
    """change button after press"""
...
def plot_it():
    ...
    aoc = (alt_o) if(alt_o_chk.get()) else(np.zeros(len(alt_o ))+1000.0)
    dsc = (dist_s)if(dist_s_chk.get())else(np.zeros(len(dist_s))+1000.0)
    asc = (alt_s) if(alt_s_chk.get()) else(np.zeros(len(alt_s ))+1000.0)  
    dmc = (dist_m)if(dist_m_chk.get())else(np.zeros(len(dist_m))+1000.0)
    amc = (alt_m) if(alt_m_chk.get()) else(np.zeros(len(alt_m ))+1000.0)  
    window_t = np.minimum(aoc,     dsc)
    window_t = np.minimum(window_t,asc)
    window_t = np.minimum(window_t,dmc)
    window_t = np.minimum(window_t,amc)
    window_t = np.reshape(window_t, ((60//step)*24,ndays), order='F')
    # im = ax.imshow(window_t, matplotlib.cm.plasma, origin="lower", extent=(0,(60//step)*24,0,(60//step)*24), vmin=0, vmax=1)#, interpolation='sinc')
    ...
    if ndays<=60:
        ...
        im = ax.imshow(window_t, matplotlib.cm.plasma, origin="lower", extent=ext, vmin=0)
    ...
    ax21.imshow(np.reshape(alt_o, ((60//step)*24,ndays), order='F'), matplotlib.cm.plasma, origin="lower", extent=ext, vmin=0)
    ...
    ax22.imshow(np.reshape(dist_s, ((60//step)*24,ndays), order='F'), matplotlib.cm.plasma, origin="lower", extent=ext, vmin=0)
    ax22.set_title("Object's distance from Sun > {} degrees".format(sun_angle))
    plt.yticks(range(0, (60//step)*24+1, (60//step)))
    plt.xticks(xticks)
    
    ax23 = plt.subplot(233)
    ax23.imshow(np.reshape(alt_s, ((60//step)*24,ndays), order='F'), matplotlib.cm.plasma, origin="lower", extent=ext, vmin=0)
    ax23.set_title("Sun's altitude < {} degrees".format(sun_alt))
    plt.yticks(range(0, (60//step)*24+1, (60//step)))
    plt.xticks(xticks)
    
    ax25 = plt.subplot(235)
    ax25.imshow(np.reshape(dist_m, ((60//step)*24,ndays), order='F'), matplotlib.cm.plasma, origin="lower", extent=ext, vmin=0)
    ax25.set_title("Object's distance from Moon > {} degrees".format(moon_angle))
    plt.yticks(range(0, (60//step)*24+1, (60//step)))
    plt.xticks(xticks)
    
    ax26 = plt.subplot(236)
    ax26.imshow(np.reshape(alt_m, ((60//step)*24,ndays), order='F'), matplotlib.cm.plasma, origin="lower", extent=ext, vmin=0)
